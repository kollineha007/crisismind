...
class CrisisService:
    ...
    def reset(self):
        self.location=LOCATIONS[0].copy(); self.data=snapshot(); self.crisis=None; self.assessment=None; self.plan=None; self.previous_plan=None; self.version=0; self.audit=[]; self.events=[]; self.active=False; self.alerts=[]; self.reports=[]; self.agent_logs={}; self.created_at=None; self.updated_at=None
    async def emit(self, agent, status, message, event_type):
        now=datetime.now(timezone.utc).isoformat(); event={"id":str(uuid4()),"timestamp":now,"event_type":event_type,"severity":"WARNING" if status=="WARNING" else "INFO","source":agent.upper().replace(" ","_"),"agent_id":agent,"agent":agent,"location":self.location["name"],"message":message,"data":{},"status":status}; self.events.append(event); self.updated_at=now; self.agent_logs.setdefault(agent,[]).append(event); await manager.broadcast(event)
