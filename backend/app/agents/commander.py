...
def build_plan(assessment, geo, shelter, medical, resource, version: int, changed: list[str] | None = None, location: str = "Vijayawada") -> ResponsePlan:
    ...
    recs = [
      ...
      Recommendation(priority="P1", action="Reserve hospital beds", reason=medical["reasoning"], affected_area=location, assigned_resource=medical["hospital"], what="Reserve beds for flood-related patients", data_used=[f"{medical['available_beds']} available beds", f"{medical['icu_beds']} ICU beds"], confidence=88, alternatives=[f"Other {location} hospitals were compared using supplied capacity"]),
